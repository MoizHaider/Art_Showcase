...
module.exports = {
  homeLoadQuery: async (args, req) => {
    ...
    const rangeLowerBound = randomPart!=0?(randomPart*2000)-2000

    const sortedPosts = await db.collection('posts').find({}).skip(10).limit(10).sort({score: -1}).toArray((err, result) => {
      if (err) {
        console.error(err);
      } else {
        return result
    ...
    console.log("This is", sortedPosts)
