...
export default async function getComments(token, postId, userId, page, newComments) {
  const newCommentsIds = newComments.map(comment=>{commentId: comment.commentId})
  const graphqlQuery = {
    query: `query getCommentsQuery($postId: ID, $userId: ID, $page: Int, $newComments: newCommentsInput){
