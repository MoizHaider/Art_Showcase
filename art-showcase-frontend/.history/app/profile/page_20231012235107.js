...
function page() {
  ...
  if(data.posts.length>0){
    data.posts.map(post=><Post type = "userPosts" _id = {post._id} likesCount = {post.likesCount} title = {post.title} description = {post.description} creationDate = {post.creationDate} commentsCount = {post.commentsCount}/>)
  }
      ...
      <div className="grid grid-cols-2">
        ...
        <main className="bg-pink-300">
          ...
          <div className="grid grid-cols-3">
            {data.posts.length>0?<Post type="userPosts" />: null}
          </div>
          <div>
