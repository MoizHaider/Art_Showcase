    ...
    <div className="max-h-[20rem] overflow-y-auto  mt-4">
      {props.newComments.length > 0
        ? props.newComments.map((commentData) => (
            <Comment commentData={commentData} key={Math.random()} tokken = {props.tokken} postId = {props.postId} />
          ))
        : null}
      {props.renderCommentSec ? (
        props.commentsData.length > 0 ? (
          props.commentsData.map((comment) => (
            <Comment commentData={comment} key={Math.random()} tokken = {props.tokken} postId = {props.postId}/>
          ))
        ) : (
          <h1>No Comments</h1>
        )
