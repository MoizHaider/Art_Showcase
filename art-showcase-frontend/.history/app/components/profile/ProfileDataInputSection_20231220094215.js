...
function ProfileDataInputSection({ type }) {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      const response = await fetch("http://localhost:8080/create-user-details", {
        method: "post",
        body: formData,
      });
  ...
  return (
    ...
    <form
      ...
      className="max-w-screen-md mx-auto p-4 bg-white shadow-md rounded-md mt-8"
    >
      <div className="mb-4">
      ...
      <div className="mb-4">
        <label htmlFor="profileImg" className="block text-sm font-semibold text-gray-600">
          Profile Image
        </label>
        <input
          type="file"
        ...
        />
        {profileImgUrl && (
          <img src={profileImgUrl} alt="Profile Preview" className="w-20 h-20 object-cover mb-2" />
        )}

        <label htmlFor="backgroundImg" className="block text-sm font-semibold text-gray-600">
          Background Image
        </label>
        <input
          type="file"
      ...
      <div className="mb-4">
        <label htmlFor="about" className="block text-sm font-semibold text-gray-600">
          About
        </label>
        <input
          type="text"
