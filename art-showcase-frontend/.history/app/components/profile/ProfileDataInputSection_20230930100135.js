...
function ProfileDataInputSection({ type }) {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);// throw an http error if a status code of 400 and 5
    }
      sessionStorage.removeItem("email"); //In fetch req if there is an http error these lines will run,
      sessionStorage.removeItem("password");// but if a network error then these line will not run and catch block will execute
    } catch(err){
      console.log(err)
    }
  };
